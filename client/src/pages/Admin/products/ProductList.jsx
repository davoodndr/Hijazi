...
import { useHandleProductArchiveMutation, useHandleProductStatusMutation, useHandleProductVisibilityMutation } from "../../../services/MutationHooks";
...
const ProductList = () => {
	...
	/* handle product listing actions */

	const productStatusMutation = useHandleProductStatusMutation();
	const productVisibilityMutation = useHandleProductVisibilityMutation();
	const productArchiveMutation = useHandleProductArchiveMutation();
	...
	const handleStatusChange = async (id, status) => {
		...
		Alert({
			...
			if (result.isConfirmed) {
				...
				const response = await productStatusMutation
					.mutateAsync({ product_id: id, status: statusChange});
	...
	const handleVisibility = (product) => {
		Alert({
			...
			if (result.isConfirmed) {
				...
				const response = await productVisibilityMutation
					.mutateAsync({ product_id: product?._id, visibility: !product.visible })
	...
	const handleMakeArchive = (product) => {
		Alert({
			...
			if (result.isConfirmed) {
				...
				const response = await productArchiveMutation
					.mutateAsync({ product_id: product?._id, archived: !product?.archived })
