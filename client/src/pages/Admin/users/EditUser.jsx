...
import { getUserInfoAction } from "../../../services/FetchDatas";
import { AnimatePresence, motion } from "motion/react";
import { useUpdateUserMutation } from "../../../services/MutationHooks";
...
const EditUser = () => {
  ...
  const getUser = async(user_id) => {
    const userData = await getUserInfoAction(user_id);
    setUser(userData);
  }
  ...
  const getAddressFields = () => {
    ...
    return Array.from(elements).map(el => el.name)
  }
  ...
  const handleSubmit = async(e) => {
    ...
    if(validateValues){
      ...
      try {
        ...
        if(showAddress && address?.length > 0 && user?.default_address){
          finalData['default_address'] = user?.default_address?._id;
        }
  ...
  return (
    <section className='flex flex-col p-6'>
      ...
      <PageTitleComponent
        title='Edit User'
        subTitle='Edit user details below'
        customActions={(
      ...
      <div className="grid grid-cols-[2fr_1fr] gap-2">
        ...
        <form onSubmit={handleSubmit} className="columns-2 gap-2 space-y-2 " id="edit-user-form">
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className='flex flex-col'>
                ...
                <div className='flex items-center border border-neutral-300 focus-within:border-primary-300 
                  ...
                  <input 
                    type={`${confirmShowing ? 'text' : 'password'}`} 
                    className='border-0!'
                    name='confirm' 
                    value={data?.confirm ?? ''} 
                    onChange={handleChange} 
          ...
          <AnimatePresence>
            {showAddress && (
              ...
              >
                <div
                  className={`break-inside-avoid space-y-6 border border-gray-200 bg-white p-6 rounded-lg shadow-xs
                  hidden-div show`}
                >
