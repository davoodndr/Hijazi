...
import { useHandleReviewStatusMutation } from "../../../services/MutationHooks";
...
function UserReviewsComponent() {
	...
	const paginatedReviews = sortedData?.slice(
		(currentPage - 1) * itemsPerPage,
		currentPage * itemsPerPage
	);

	/* handling action buttons */
	const statusChangeMutation = useHandleReviewStatusMutation();
	...
	const handleStatusChange = async (id, status) => {
		...
		Alert({
			...
			if (result.isConfirmed) {
				...
				const response = await statusChangeMutation
					.mutateAsync({ review_id: id, status: statusChange });
