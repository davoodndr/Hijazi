...
function createZoomImageHover(container, options) {
  ...
  const finalOptions = {
    zoomImageSource: options.zoomImageSource ?? null /* || sourceImgElement.src */,
...
function ImageZoomOnHover(
  ...
  useEffect(() => {
    ...
    if (
      ...
      imageHoverContainerRef.current &&
  ...
  useEffect(() => {
    const updateTop = () => {
      if (zoomTargetRef.current) {
        zoomTargetRef.current.style.top = `${window.scrollY - 30}px`;
      }
    };

    updateTop(); // initial position
  ...
  return (
    <>
      ...
      >
        <img
          className="EasyZoomHoverSmallImage w-full smooth"
          loading='lazy'
