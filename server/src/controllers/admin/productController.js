...
export const addProduct = async(req, res) => {
  ...
  const { info } = req.body;
  const { name, sku, slug, price, stock, tax, description, category, brand, variants } = JSON.parse(info);
  ...
  try {
    ...
    const newProduct = await Product.create(JSON.parse(info));
    await uploadImages(req, newProduct);

    const updated = await Product.findByIdAndUpdate(newProduct?._id, 
...
const uploadImages = async(req, product) => {

  const { remove_ids, folder } = req?.body;

  const productImages = req?.files['productImages'] || [];
  const variantImages = req?.files['variantImages'] || [];
  //console.log(productImages, variantImages, info, remove_ids, folder)
...
export const updateProduct = async(req, res) => {
  ...
  const { info } = req.body;
  const { product_id, name, sku, slug, price, stock, tax, description, category, brand, variants } = JSON.parse(info);
  ...
  try {
    ...
    await uploadImages(req, product) 
